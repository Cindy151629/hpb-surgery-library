...
WORK=Path(os.environ.get('HPB_WORK_DIR',str(ROOT.parents[1]/'work/hpb')))
  ...
  try:
   for redirect in range(8):
    ...
    with lock:
     ...
     r=requests.get(url,timeout=timeout,allow_redirects=False,headers={'User-Agent':'HPB-Research-Library/1.0 (educational bibliography verification)'})
