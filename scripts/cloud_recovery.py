...
STATE_DIRS = ('data/reports', 'data/runs', 'data/checkpoints')
GOOD_REF = 'refs/heads/release-good'
BOT_ENV = {
    'GIT_AUTHOR_NAME': 'hpb-library-bot', 'GIT_AUTHOR_EMAIL': 'hpb-library-bot@users.noreply.github.com',
    'GIT_COMMITTER_NAME': 'hpb-library-bot', 'GIT_COMMITTER_EMAIL': 'hpb-library-bot@users.noreply.github.com',
}
...
def persist(root, branch, remote='origin'):
    ...
    commit = git(root, 'commit-tree', tree, '-p', head,
                 content=b'Persist HPB cumulative progress and runtime outcome [skip ci]\n').decode().strip()
...
def validate_site(files, receipt):
    if set(files) != set(SITE_FILES):
        raise ValueError('release must contain only the four approved site files')
    envelope = json.loads(files['data.json'])
    version = json.loads(files['version.json'])
    payload = json.loads(envelope['payload'])
    expected = receipt.get('data_version')
    if (not receipt.get('deployed') or not receipt.get('verified_at') or not receipt.get('cors_origin_null')
            or receipt.get('domain_id') != 'hpb-surgery'):
        raise ValueError('release has no successful external verification receipt')
    if (envelope.get('data_version') != expected or version.get('data_version') != expected
            or payload.get('data_version') != expected or envelope.get('domain_id') != 'hpb-surgery'
            or envelope.get('schema_version') != 1 or payload.get('domain_id') != 'hpb-surgery'
            or payload.get('schema_version') != 1 or digest(envelope['payload']) != envelope.get('sha256')
            or envelope.get('sha256') != receipt.get('sha256')):
        raise ValueError('release version or payload checksum mismatch')
    for name in ('index.html', 'data.json'):
        if digest(files[name]) != receipt.get('file_hashes', {}).get(name):
            raise ValueError('site bytes were not externally verified: ' + name)
    if expected.encode() not in files['index.html']:
        raise ValueError('release HTML version mismatch')
...
def pin_good(root, site, remote='origin'):
    ...
    manifest = {'schema_version': 1, 'domain_id': 'hpb-surgery', 'receipt': receipt,
    ...
    commit = git(root, 'commit-tree', tree, *parent,
                 content=('Externally verified HPB release ' + receipt['data_version'] + '\n').encode()).decode().strip()
...
def start_runtime(root):
    ...
    result = {
        'domain_id': 'hpb-surgery', 'schema_version': 1, 'status': 'running',
...
def finish_runtime(root, step_results):
    ...
    runtime.update(finished_at=now(), steps=steps,
                   ...
                   status='failed' if failed else ('partial' if state.get('last_attempt_status') == 'partial' else 'success'),
...
def main():
    ...
    if args.action == 'authorize':
        ...
        result = finish_runtime(ROOT, json.loads(os.environ.get('HPB_STEP_RESULTS', '{}')))
