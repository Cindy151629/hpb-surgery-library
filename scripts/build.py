...
def build(migration=False,output=None):
 ...
 stable={'domain_id':'hpb-surgery','schema_version':1,'records':records,'candidates':candidates,'candidate_sources':candidate_sources,'taxonomy':tax,'coverage':cov,'state':state,'deployment':dep,'methods':read(ROOT/'config/sources.json')};version='hpb-'+digest(json.dumps(stable,ensure_ascii=False,sort_keys=True))[:16];payload={**stable,'data_version':version,'generated_at':stamp};raw=json.dumps(payload,ensure_ascii=False,separators=(',',':'));envelope={'domain_id':'hpb-surgery','schema_version':1,'data_version':version,'sha256':digest(raw),'payload':raw};write(ROOT/'data/library-envelope.json',envelope)
 src=ROOT/'src';html=(src/'index.html').read_text().replace('/*__CSS__*/',(src/'style.css').read_text()).replace('/*__DATA__*/',raw.replace('<','\\u003c').replace('\u2028','\\u2028').replace('\u2029','\\u2029')).replace('/*__JS__*/',(src/'app.js').read_text());dest=Path(output) if output else ROOT/'肝胆胰外科_文献与视频笔记库.html';dest.parent.mkdir(parents=True,exist_ok=True);dest.write_text(html)
