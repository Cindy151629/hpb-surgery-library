...
def reports():
 ...
 lines=['# 肝胆胰资源逐条核验报告','',f'生成时间：{summary["generated_at"]}；数据版本：{summary["data_version"]}。', '',f'共 {len(records)} 条主记录，{attempts} 条具有本轮原始链接请求记录。核心身份确认 {summary["core_identity_confirmed"]}；目标题名匹配页面 {summary["page_title_matched"]}；已取得并匹配全文 {summary["matched_fulltext_obtained"]}。这些维度不相互替代。', '',f'实质内容笔记 {summary["substantive_notes"]} 条；资料受限：{", ".join(limited)}。摘要导读不是全文精读。原站链接均保留；视频播放或嵌入未通过实测的，不提供站内播放按钮。', '', '| ID | 类型 | 原始题名 | 核心题录 | 页面 | 阅读范围 | 核验结论 | 本轮检查 |','|---|---|---|---|---|---|---|---|']
